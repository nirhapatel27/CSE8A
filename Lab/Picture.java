 ...
 public void flipVerticalRectangle(int x, int y, int width, int height)
  {
    ...
    for(int j=0; j < limit2/2; j++)
    {
      ...
      {
        //FILL THIS OUT AND THEN UNCOMMENT IT!
        source=getPixel(x+i,j+y-height+1);
        destination=getPixel(x+i,y-j);
        tempColor = destination.getColor();
        temp.setColor(source.getColor());
        source.setColor(tempColor);
        destination.setColor(temp.getColor());
  ...
  public void flipHorizontalRectangle(int x, int y, int width, int height)
  {
    ...
    for(int j=0; j < limit2; j++)
    {
      ...
      {
        //FILL THIS OUT AND THEN UNCOMMENT IT!
        source=getPixel(x-i+width-1,y-j);
        destination=getPixel(x+i,y-j);
        tempColor = destination.getColor();
        temp.setColor(source.getColor());
        source.setColor(tempColor);
  ...
  /*public void scramble()
  {
     ...
     int randomWidth=randomGenerator.nextInt(8);
     Picture picture1=new Picture(fileName);
     for(int sourceX = 0, targetX = 0; sourceX< this.getWidth()/3;sourceX++,targetX++)
     {
       for(int sourceY = 0, targetY=0; sourceY< this.getHeight()/3;sourceY++,targetY++)
           {
         sourcePixel=pictures[0].getPixel(sourceX,sourceY);
         targetPixel = this.getPixel(targetX,targetY);
         targetPixel.setColor(sourcePixel.getColor());
           }
